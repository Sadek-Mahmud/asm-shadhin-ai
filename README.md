...
## 📊 Empirical Verification & Zero-Day Statistical Proof (98.64% TPR)
...
* **Zero-Day Detection Rate (TPR):** **98.64%** (10M Flows, Wilson Score 95% CI: `[98.61%, 98.67%]`, $p < 0.001$) | **98.40%** (1.28M Academic Benchmark, CI: `[98.18%, 98.62%]`)
* **False Positive Rate (FPR):** **0.12%** on 10M flows | **< 1.14%** on edge-case benchmarks
* **Overall Classification Accuracy:** **99.50%**
* **Mitigation Latency:** **0.33 µs ($p_{50}$)** line-rate eBPF drop | **0.92 µs ($p_{90}$)** | **20.79 µs ($p_{99}$)** worst-case full-stack entropy path
